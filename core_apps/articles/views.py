...
class ArticleListCreateView(generics.ListCreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        logger.info(
            f"article {serializer.data.get('title')} is created by {self.request.user.first_name.title()}"
        )
...
class ClapArticleView(generics.CreateAPIView, generics.DestroyAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        return Response(
            {"detail": "clap is added to article."}, status=status.HTTP_201_CREATED
        )
    ...
    def delete(self, request, *args, **kwargs):
        ...
        return Response(
            {"detail": "clap is removed from article."},
